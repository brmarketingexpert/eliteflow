...
system_create_user() {
  ...
  sudo su - root <<EOF
  useradd -m -p $(openssl passwd $deploy_password) -s /bin/bash -G sudo deployplw
  usermod -aG sudo deployplw
  mv "${PROJECT_ROOT}"/izing.zip /home/deployplw/
EOF
...
system_set_user_mod() {
  ...
  sudo su - root <<EOF
  sudo usermod -aG docker deployplw
  su - deployplw
EOF
...
system_success() {
  ...
  printf "\n"
  printf "Senha Usuario Deployplw: $deploy_password"
  printf "\n"
