  ...
  mutations: {
    ...
    UPDATE_TICKET(state, payload) {
      ...
      if (ticketIndex !== -1) {
        ...
        // console.log('1', state.tickets.sort((a, b) => b.lastMessageAt - a.lastMessageAt).map(ticket => {
        //   return { id: ticket.id, lastMessageAt: ticket.lastMessageAt, lastMessage: ticket.lastMessage, updatedAt: ticket.updatedAt };
        // }).slice(0, 4))

        // setTimeout(async () => {
        //   const ticketIds = state.tickets.map(ticket => {
        //     return { id: ticket.id, lastMessageAt: ticket.lastMessageAt, lastMessage: ticket.lastMessage, updatedAt: ticket.updatedAt };
        //   }).slice(0, 4);
        //   ticketIds.sort((a, b) => b.lastMessageAt - a.lastMessageAt);
        //   const consultas = tickets.map(async (ticket) => {
        //     const dados = await ConsultarDadosTicket(ticket);
        //     return {id: dados.data.id, lastMessageAt: dados.data.lastMessageAt, lastMessage: dados.data.lastMessage}
        //   }).slice(0, 4);
        //   const dadosParaTodosTickets = await Promise.all(consultas);
        //   console.log('2', dadosParaTodosTickets.sort((a, b) => b.lastMessageAt - a.lastMessageAt));
        //   console.log('3', ticketIds);
        // }, 1000); 
