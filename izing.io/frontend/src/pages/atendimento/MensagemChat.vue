        ...
        >
          ...
          <div style="min-width: 100px; max-width: 350px" :style="mensagem.isDeleted ? 'color: rgba(0, 0, 0, 0.36) !important;' : ''">
            ...
            <q-btn v-if="!mensagem.isDeleted && isShowOptions" class="absolute-top-right" dense flat ripple round icon="mdi-chevron-down">  
              <q-menu square auto-close anchor="bottom left" self="top left">
                <q-list style="min-width: 100px">
                  <q-item :disable="!['whatsapp', 'telegram'].includes(ticketFocado.channel)" clickable @click="citarMensagem(mensagem)"  v-if="ticketFocado.channel === 'whatsapp' && mensagem.mediaType !== 'notes'">
                  ...
                  <q-item :disable="!['whatsapp', 'telegram'].includes(ticketFocado.channel)" clickable @click="mensagemReacao = mensagem; modalEmojiOpen = true" v-if="mensagem.mediaType !== 'notes'  && ticketFocado.channel === 'whatsapp'">
                    <q-item-section>Reagir</q-item-section>
                  </q-item>
                  <q-item clickable @click="abrirModalEdicao(mensagem)" v-if="mensagem.mediaType === 'chat' && ticketFocado.channel === 'whatsapp'">
                    <q-item-section>Editar Mensagem</q-item-section>
                  </q-item>
                  <q-separator />
                  <q-item @click="deletarMensagem(mensagem)" clickable v-if="mensagem.fromMe && mensagem.mediaType !== 'notes'" :disable="isDesactivatDelete(mensagem) || ticketFocado.channel === 'messenger'">
                    <q-item-section>
            ...
            <template v-if="mensagem.mediaType === 'interactive'">
              <div v-html="formatarMensagemRespostaBotaoWhatsapp(mensagem.body)"></div>
            </template>
            <template v-if="mensagem.mediaType === 'button'">
              <div v-html="formatarBotaoWhatsapp(mensagem.body)"></div>
            </template>
            ...
            <template style="margin-top:20px" v-if="!['audio', 'vcard', 'image', 'video', 'sticker', 'location', 'interactive', 'button', 'button_reply', 'sticker', 'notes'].includes(mensagem.mediaType) && mensagem.mediaUrl">
            ...
            <template v-if="['image', 'video'].includes(mensagem.mediaType) && mensagem.mediaUrl" style="margin-top:20px">
              <div class="text-center full-width hide-scrollbar no-scroll">
                <q-btn
                  type="a"
                  :color="$q.dark.isActive ? '' : 'grey-3'"
            ...
            <div
              ...
              v-linkified
              v-if="!['vcard', 'application', 'audio', 'button', 'location', 'interactive', 'button_reply', 'sticker', 'notes', 'templates'].includes(mensagem.mediaType)"
