<template>
  <div @drop.prevent="handleInputDrop">
    <div ref="menuFast">
      <q-menu
    ...
    <div class="q-py-md row bg-white justify-start items-center text-grey-9 relative-position">
      <template v-if="!isRecordingAudio">
        ...
        <q-btn
          v-if="$q.screen.width > 500 && ticketFocado.channel === 'whatsapp'"
          flat
          dense
...
import { SendGhostMessage, SendMentionMessage, SendMentionAllMessage, ListParticipants } from 'src/service/tickets'
import { ListarConfiguracoes } from 'src/service/configuracoes'
import { Texto } from 'src/service/massa'
import { EnviarTextoWaba, EnviarArquivoWaba, EnviarBotaoWaba, EnviarArquivoUrlWaba, BuscarTemplates, EnviarTemplateWaba } from 'src/service/waba'
...
export default defineComponent({
  ...
  methods: {
    ...
    // onClickResumirConversa() {
    //   this.$q.dialog({
    //     component: () => import('./DialogGPTResumirConversa.vue'),
    //     parent: this,
    ...
    //         id: item.id,
    //         fromMe: item.fromMe,
    //         sender: item.fromMe ? localStorage.getItem('username') : item.contact?.name || item.ticket?.contact?.name || 'desconhecido',
    //         body: item.body,
    //       })),
    //   })
    // },
    // onClickSentimentoConversa() {
    //   this.$q.dialog({
    //     component: () => import('./DialogGPTSentimentoConversa.vue'),
    //     parent: this,
    //     mensagens: this.mensagens
    //       .filter((item) => !!item.body.length)
    //       .slice(-30)
    ...
    // onClickChatGPT() {
    //   this.$q.dialog({
    //     component: () => import('./DialogIntegracaoGPT.vue'),
    //     parent: this,
    //   })
    // },
