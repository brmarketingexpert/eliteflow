    ...
    <q-layout class="WAL__layout shadow-3" container view="lHr LpR lFr">
      ...
      >
        ...
        <q-toolbar v-show="toolbarSearch" class="row q-gutter-sm q-py-sm items-center">
          ...
          <q-btn flat class="bg-padrao btn-rounded" icon="mdi-text-box-plus-outline" @click="showNotification()" v-if="hasMoreTickets"  style="flex: 1;">
          ...
          <div class="q-pa-sm">
            ...
            <q-card class="bg-white btn-rounded q-mt-sm" style="width: 100%" bordered flat>
              <q-card-section class="text-center">
                ...
                <q-btn flat class="btn-outline btn-small" label="Editar Contato" @click="editContact(ticketFocado.contact.id)" />
            ...
            <q-card class="bg-white btn-rounded q-mt-sm" style="width: 100%" bordered flat v-if="ticketFocado.channel !== 'waba'">
              <q-card-section class="text-bold q-pa-sm">
            ...
            <q-card class="bg-white q-mt-sm btn-rounded" style="width: 100%" bordered flat :key="ticketFocado.id + $uuid()"  v-if="dialogflowAtivo === 'enabled' && ticketFocado.channel !== 'waba'">
              <q-card-section class="q-pa-none">
                <q-item tag="label" v-ripple>
                  <q-item-section>
...
import { ConsultarDadosTicket, ConsultarLogsTicket, ConsultarTickets, DeletarMensagem, AtualizarTypebotTicket, AtualizarDialogflowTicket, AtualizarChatgptTicket, EnviarMensagemTexto, AtualizarStatusTicket, AtualizarStatusTicketNull } from 'src/service/tickets'
...
import { SyncOldMessagesByUserWbot } from 'src/service/tickets'
...
export default {
  ...
  computed: {
    ...mapGetters(['tickets', 'ticketFocado', 'mensagens', 'hasMore', 'whatsapps']),
    cUserQueues() {
      return UserQueues
    },
    style() {
  ...
  methods: {
    async aceitarTermos(){
      ...
      const hasFalseAcceptTerms = data.some(tenant => !tenant.acceptTerms);
      if (hasFalseAcceptTerms) {
        this.$q.notify({
          type: 'negative',
          message: 'Solicite ao Super Administrador o aceite dos termos de uso.',
          progress: true,
    ...
    async carregarIntegracaoStatus() {
      if (this.ticketFocado && (this.ticketFocado.typebotStatus !== undefined || this.ticketFocado.dialogflowStatus !== undefined || this.ticketFocado.chatgptStatus !== undefined)) {
        setTimeout(async () => {
          try{
